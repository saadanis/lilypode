    ...
    func authorize() async {
        do {
            if HKHealthStore.isHealthDataAvailable() {
                try await healthStore.requestAuthorization(toShare: [HKQuantityType.stateOfMindType()], read: [HKQuantityType.stateOfMindType()])
            }
        } catch {
...
struct K {
    static var entries: [Entry] {
        ...
        .init(timestamp: calendar.date(byAdding: .day, value: -18, to: Date())!,
